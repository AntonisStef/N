...
import numpy as np
import OutOfBolometryGridException
...
class BolometryTable:
	...
	def interpolate(self,value):
		...
		for i in range(nParams):
			if value[i] < nearGridNode[i]:
				prevIndex=self.previousNode(value,i)
				if prevIndex==-1:
		...
		for i in range(nElems):
			...
			try:
				for j in range(nParams):
					if val%2==1:
						...
						if not j in v:
							...
							if nextNode==-1:
								nextNode=self.hextNode(nearGridNode,j)
								if nextNode==-1:
									raise ValueError()
							v[j]=nextNode
						nodeCopy[j]=self.__param[nextNode][j]
					val=val/2
						
				index=self.where(nodeCopy)
					
				if self.where(nodeCopy)==-1:
					raise ValueError()
				...
				if index==-1:
					raise ValueError()
				hypercubeNode.append(nearGridNode)
				hypercubeBc.append(self.__bc[index])
		...
		for i in range(nParams):
			...
			if hypercubeNodes[2**i][i]==nodeCopy[i]:
				x[i]=0
			else:
				x[i]=(value[i]-nodeCopy[i]) / (hypercubeNodes[2**i][i]-nodeCopy[i])
