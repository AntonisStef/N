...
class TestBcg(unittest.TestCase):
	...
	def test_random(self):
		...
		table=pd.read_table('data/test/bcgran.txt', delim_whitespace=True, names=["teff", "logg","metal","alpha","bc"],dtype={'teff':np.float64,'logg':np.float64,'metal':np.float64,'alpha':np.float64,'bc':np.float64})
